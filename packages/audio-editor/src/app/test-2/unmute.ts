/**
 * @file unmute.ts
 * @description This file helps in unmuting the Web Audio context and handling various related functionalities.
 */
...
export function unmute(
  ...
  function updatePlaybackState(): void {
    ...
    const shouldAllowPlayback: boolean =
      ...
      ((!pageVisibilityAPI ||
        !document[pageVisibilityAPI.hidden as keyof Document]) && // can be active if no page vis api, or page not hidden
      ...
      if (allowPlayback) {
        ...
        if (isMediaPlaybackEvent) {
          ...
          if (!channelTag) {
            ...
            (channelTag as any).disableRemotePlayback = true; // Airplay like controls on other devices, prevents casting of the tag, doesn't work on modern iOS
  ...
  /**
   * Completely unloads / destroys the channel tag.
   */
